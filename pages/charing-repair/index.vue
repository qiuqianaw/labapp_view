      ...
      <view v-else>
        <van-tabs swipeable color="#1f8dd6d2">
          <van-tab :title="'报修中(' + repairLength + ')'">
            ...
            <repair
              :repair="repair"
              :repairImgsList="repairImgsList"
              :loading="loading"
              @random="randNum($event)"
            ></repair>
          </van-tab>
          <van-tab :title="'已确认(' + confirmedLength + ')'">
            ...
            <confirmed
              :confirmed="confirmed"
              :confirmedImgsList="confirmedImgsList"
              :loading="loading"
              @random="randNum($event)"
            ></confirmed>
          </van-tab>
          <van-tab
            :title="'已解决(' + (repairedLength + unrepairedLength) + ')'"
          >
            <van-notice-bar
              color="#1989fa"
              background="#ecf9ff"
            ...
            <view class="margin-top-sm">
              <van-tabs class="margin-top" color="#1f8dd6d2" type="card">
                <van-tab :title="'已修复(' + repairedLength + ')'">
                  <repaired
                    :repaired="repaired"
                    :repairedImgsList="repairedImgsList"
                    :loading="loading"
                    @random="randNum($event)"
                  ></repaired>
                </van-tab>
                <van-tab :title="'已报废(' + unrepairedLength + ')'">
                  <unrepaired
                    ...
                    :unrepairedImgsList="unrepairedImgsList"
                    :loading="loading"
                    @random="randNum($event)"
                  />
                </van-tab>
...
import { listByCharingRepair } from "@/api/module.js";
export default {
  components: {
    repair: repair,
    repaired: repaired,
  ...
  methods: {
    getData(status) {
      listByCharingRepair(status).then((res) => {
          ...
          for (var index in res.data.data) {
            if (res.data.data[index].imgs != null) {
              for (var index2 in res.data.data[index].imgs) {
                if (status == 1) {
                  this.repairImgsList.push(
                    res.data.data[index].imgs[index2].filepath
                  );
                } else if (status == 2) {
                  this.confirmedImgsList.push(
                    res.data.data[index].imgs[index2].filepath
                  );
                } else if (status == 3) {
                  this.repairedImgsList.push(
                    res.data.data[index].imgs[index2].filepath
                  );
                } else {
                  this.unrepairedImgsList.push(
                    res.data.data[index].imgs[index2].filepath
                  );
                }
              }
