<template>
	<view>
		...
		<view v-else class="cu-card">
			<view class="cu-item shadow padding-top-sm" v-if="confirmed.length == 0">
				<van-empty description="暂无报修单" />
			</view>
			<view class="cu-item shadow padding-top-sm" v-for="(item,index) in confirmed" :key="index" v-else>
				<van-cell icon="todo-list-o">
					<template #title>
						 <view class="custom-title">报修编号: {{ item.repid }}</view>
						 <span class="margin-right-sm"><van-tag type="primary">处理人编号:{{item.operateuser}}</van-tag></span>
						 <span class="margin-right-sm"><van-tag type="primary">处理人姓名:{{item.operateusername}}</van-tag></span>
						 <span class="margin-right-sm"><van-tag plain type="primary">操作时间:{{item.operatetime}}</van-tag></span>
						 <span v-if="item.operatedesc != null" class="margin-right-sm"><van-tag plain type="primary">处理描述:{{item.operatedesc}}</van-tag></span>
					</template>
				</van-cell>
...
<script>
	...
	export default{
		props: ['confirmed','confirmedImgsList'],
		data(){
			return{
		...
		methods: {
			toPreview(event){
				...
				uni.previewImage({
					 ...
					 urls: this.confirmedImgsList // 需要预览的图片http链接列表
				})
			}
		}
	}
