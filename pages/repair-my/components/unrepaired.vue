<template>
	<view>
		<view class="cu-card">
			...
			<view class="cu-item shadow padding-top-sm" v-for="(item,index) in unrepaired" :key="index" v-else>
				<van-cell icon="todo-list-o">
					<template #title>
						 <view class="custom-title">报修编号: {{ item.repid }}</view>
						 <span class="margin-right-sm"><van-tag type="danger">处理人编号:{{item.operateuser}}</van-tag></span>
						 <span class="margin-right-sm"><van-tag type="danger">处理人姓名:{{item.operateusername}}</van-tag></span>
						 <span class="margin-right-sm"><van-tag plain type="danger">操作时间:{{item.operatetime}}</van-tag></span>
						 <span v-if="item.operatedesc != null" class="margin-right-sm"><van-tag plain type="danger">处理描述:{{item.operatedesc}}</van-tag></span>
					</template>
				</van-cell>
