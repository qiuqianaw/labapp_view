<template>
	<view>
		...
		<view v-else>
			...
			<view class="cu-card">
				<view class="cu-item shadow padding-xl" v-if="LabsaInfoList.length == 0">
				...
				<view class="cu-item shadow padding-xl" v-else>
					...
					<view class="margin-top-sm text-df" v-for="(item, index) in LabsaInfoList" :key="index">
						<view class="text-gray" v-if="item.sastatus == 0">
							<van-row>
								<view>
									<van-col class="margin-top-xl" span="18">{{
                    item.saname
                  }}</van-col>
								</view>
								<view>
